...
data2 <- read.csv2("MetabolomeMoms_New.csv", fileEncoding = "latin1", sep = ",")


# Extract columns from 12th to the last in data2
data2_subset <- data2[, 12:ncol(data2)]

# Add STUDY.ID to the subset so we can merge
data2_subset$STUDY.ID <- data2$STUDY.ID

# Merge data1 and data2 based on STUDY.ID
merged_data <- merge(data, data2_subset, by = "STUDY.ID", all.x = FALSE)

# which(colnames(merged_data)=="M75T402_2_POS_RPLC")  ### first metabolome
...
###### 
write.csv(data, file = "Lipid_Metabolome_mom_PTB.csv", row.names = FALSE)
