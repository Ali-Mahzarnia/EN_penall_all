...
data <- read.csv("Lipid_Metabolome_mom_PTB.csv")
...
while (is.null(cvfit)) {
  tryCatch({
    ...
    cvfit <- cv.glmnet(X[train.index,], y[train.index], family = "binomial", 
                       alpha = best_alpha, type.measure = 'auc' , 
                       nfolds = 5)
...
output_file <- "EN_Lipid_Metabolome_PTB_AUCs.xlsx"
...
# extract the coolumn name of metabolome from its first column of metabomole 
colnames_metabolome = colnames(X)[which( colnames(X) == "M75T402_2_POS_RPLC" ):ncol(X)]
...
# Separate Lipids (where Feature is NOT in colnames_metabolome)
lipids_features <- selected_features_data[!selected_features_data$Feature %in% colnames_metabolome, ]

# Separate Metabolome (where Feature IS in colnames_metabolome)
metabolome_features <- selected_features_data[selected_features_data$Feature %in% colnames_metabolome, ]


#order
lipids_features <- lipids_features[order(lipids_features$Frequency, decreasing = T), ]
metabolome_features <- metabolome_features[order(metabolome_features$Frequency, decreasing = T), ]

# Create a new Excel file with two sheets
output_file <- "EN_Lipid_Metabolome_features_PTB.xlsx"

# Write lipids_features to the "Lipids" sheet
write.xlsx(lipids_features, file = output_file, sheetName = "Lipids", row.names = FALSE)

# Append metabolome_features to the "Metabolome" sheet
write.xlsx(metabolome_features, file = output_file, sheetName = "Metabolome", append = TRUE, row.names = FALSE)
